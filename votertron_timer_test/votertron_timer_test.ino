...
void loop() {
  ...
  if (init_print == 0) {
    delay(200);
    Serial.println("Select your state with the scroll wheel");
    init_print = 1;
  }
...
void state_question() {
  ...
  if (current_button != lastButtonState) {
    ...
    if (!state_select) {
      ...
      Serial.print("State: ");
      delay(50);
      Serial.print(states[filtered_scroll]);
      Serial.println("");
    }
  }
  ...
  if (state_select) {
    if (next_print == 0) {
      Serial.println("You selected the state of");
      Serial.print(states[my_state]);
      Serial.println("");
      next_question = true;
      next_print = 1;
    }
...
void race_question() {
  ...
  if (current_button_state != lastButtonState2) {
    ...
    if (!race_select) {
      ...
      Serial.print("Race: ");
      delay(50);
      Serial.print(races[filtered_scroll]);
      Serial.println("");
    }
  }
  ...
  if (race_select) {
    if (third_print == 0) {
      Serial.println("You selected your race as: ");
      Serial.print(races[my_race]);
      third_print = 1;
    }
  }
